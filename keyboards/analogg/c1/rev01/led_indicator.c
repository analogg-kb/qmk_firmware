...
void led_indicator_set_ble_to(uint8_t tunnel) {
    switch (tunnel) {
        case 0:
            led_indicator.ble = indicator_set_rgb(RGB_BLUE);
            break;
        case 1:
            led_indicator.ble = indicator_set_rgb(RGB_GREEN);
            break;
        case 2:
            led_indicator.ble = indicator_set_rgb(RGB_YELLOW);
            break;
        case 3:
            led_indicator.ble = indicator_set_rgb(RGB_MAGENTA);
            break;
        case 4:
            led_indicator.ble = indicator_set_rgb(RGB_RED);
            break;
        default:
            break;
    }
...
void rgblite_setrgb(_led_indicator led_indicator) {
    LED_TYPE leds[RGBLED_NUM] = {led_indicator.caps_lock, led_indicator.ble, led_indicator.battery_level, led_indicator.power};
